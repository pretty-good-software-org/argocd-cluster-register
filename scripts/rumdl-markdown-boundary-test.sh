...
grep -Fxq 'enabled = false' .rumdl.toml || {
  printf '.rumdl.toml must disable MD060 by default\n' >&2
  exit 1
}
...
if rumdl check --deny-config-warnings --extend-enable MD060 --config 'MD060.enabled = true' -- "$work/mutation.md"; then
